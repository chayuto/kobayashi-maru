...
After extracting all managers, we need to:
1. Rewrite Game.ts to use the new architecture
2. Ensure all functionality is preserved
3. Keep the public API unchanged
4. Reduce Game.ts to ~150 lines

---

## Solution: Game as Facade

Transform Game.ts into a thin facade that:
- Orchestrates manager initialization
...
### File: `src/core/Game.ts` (Rewritten)
...
import { createRenderSystem, createMovementSystem, createCollisionSystem, createTargetingSystem, createCombatSystem, createDamageSystem, createAISystem, createProjectileSystem, statusEffectSystem, createEnemyCollisionSystem, createEnemyCombatSystem, createEnemyProjectileSystem, createAbilitySystem } from '../systems';
...
export class Game {
  ...
  private registerSystems(): void {
    ...
    // Store combat system reference for beam rendering
    (this as { combatSystem?: typeof combatSystem }).combatSystem = combatSystem;
  }
  ...
  private setupLoopCallbacks(): void {
    ...
    this.loopManager.onRender((dt) => {
      ...
      const combatSystem = (this as { combatSystem?: { getActiveBeams(): unknown[] } }).combatSystem;
      this.renderManager.render(combatSystem?.getActiveBeams() ?? []);
    });
    ...
    this.loopManager.onUI(() => {
      ...
      const combatSystem = (this as { combatSystem?: unknown }).combatSystem;
      
      this.uiController.updateHUD(snapshot, combatSystem, turretCount);
  ...
  getCollisionSystem() { return getServices().tryGet('collisionSystem'); }
  ...
  getCombatSystem() { return (this as { combatSystem?: unknown }).combatSystem; }
  getDamageSystem() { return null; } // TODO: Store reference
...
| Constructor | 30 | 5 |
| init() | 200 | 30 |
| update() | 150 | 0 (in callbacks) |
| Event handlers | 80 | 0 (in managers) |
| Public API | 200 | 50 |
| **Total** | **~1164** | **~250** |
...
- [ ] Game.ts compiles without errors
...
1. **BACKUP** the original `src/core/Game.ts`
...
If the new Game.ts has issues:

1. Restore from backup
2. Identify which manager has the bug
3. Fix the manager in isolation
...
- **Task 09: Integration Testing** - Comprehensive testing of the refactored system

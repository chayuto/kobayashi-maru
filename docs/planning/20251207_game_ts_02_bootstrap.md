...
Game.ts `init()` method is 200+ lines of sequential initialization:

```typescript
async init(): Promise<void> {
  // 1. PixiJS initialization (20 lines)
  await this.app.init({ ... });
  this.container.appendChild(this.app.canvas);
  
  // 2. Touch/Input setup (10 lines)
  this.touchInputManager = new TouchInputManager(this.app);
  this.touchInputManager.init();
  
  // 3. Audio initialization (15 lines)
  const initAudio = () => { ... };
  window.addEventListener('click', initAudio);
  
  // 4. Starfield initialization (5 lines)
  if (this.starfield) { this.starfield.init(multiplier); }
  
  // 5. Rendering systems (50 lines)
  this.spriteManager.init();
  this.beamRenderer = new BeamRenderer(this.app);
  // ... many more
  
  // 6. Glow layers setup (30 lines)
  this.glowManager = new GlowManager();
  // ... layer setup
  
  // 7. ECS systems (40 lines)
  this.collisionSystem = createCollisionSystem(this.spatialHash);
  // ... many more
  
  // 8. UI initialization (30 lines)
  this.hudManager.init(this.app, this);
  // ... callbacks
  
  // 9. Gameplay initialization (10 lines)
...
- AI agents struggle with 200+ line methods

---

## Solution: GameBootstrap Class

Extract initialization into a dedicated bootstrap class that:
- Registers all services with ServiceContainer
- Handles async initialization in clear phases
- Provides progress callbacks for loading screens
...
### File: `src/core/bootstrap/GameBootstrap.ts`
...
class Game {
  async init(): Promise<void> {
    ...
    // All services now available via getServices()
    const services = getServices();
    
    // Initialize HUD with game reference
    services.get('hudManager').init(app, this);
...
- [ ] GameBootstrap compiles without errors
- [ ] All services are registered in correct order
- [ ] Glow layers are added to stage correctly
- [ ] Audio initialization deferred to user interaction
- [ ] Progress callback works for loading screens
...
1. Create `src/core/bootstrap/` directory
2. Create `GameBootstrap.ts` with the code above
3. Create `index.ts` barrel export
4. Update `src/core/index.ts` to export bootstrap
...
- **Task 03: Game Loop Extraction** - Extracts update loop into GameLoopManager
